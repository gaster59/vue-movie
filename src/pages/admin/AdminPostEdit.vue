...
export default {
  ...
  async created () {
    ...
    const response = await shared.getDetailPost(postId)
    const data = response.data
    const detailPost = data.post
    this.title = detailPost.title
    this.summary = detailPost.summary
    this.content = detailPost.content
    this.category = data.postCategory.toString()
    this.avatar = 'http://blog-management.test:90/post/' + detailPost.avatar
    console.log(data)
  ...
  methods: {
    ...
    async onSubmit (evt) {
      ...
      let result = await shared.editPost(post)
      console.log(result)
      this.isLoading = false
    }
  }
